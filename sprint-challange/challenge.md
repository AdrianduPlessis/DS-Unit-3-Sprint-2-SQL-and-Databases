...
- In the Northwind database, what is the type of relationship between the
  ...
  The relationship between the `Employee` and `Territory` tables is many-to-many,
  joined by the junction table `EmployeeTerritory`. This is because each employee might have
  multiple territories and territories are not necessarily exclusive to a certain employee.
  Each key value in `Employee` potentially maps to many (one-to-many) entries in 
  the junction table, which in turn maps to `Territory` with a many-to-one relationship and
  vice versa. Though the junction table might initially seem like an unnecessary intermediary step,
  it's crucial to avoid convoluting tables with arbitrary retrofitted columns to handle unexpected
  situations.
  
- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?
    
  There are two primary draws to document stores, one being scalability and the other
  adaptability. If you are a startup in the tech industry and you're not harvesting
  as much (ethical) data as is available to you, you're already losing, but that data
  might not be immediately relevant to your goal, or predictable in structure. If you
  want to store large amounts of data and concern yourself with parsing it later, that's
  where a document store takes the clear lead. Data doesn't have to fit on expensive RAM,
  and can essentially scale indefinitely.
  On the other hand, we might consider the example of a bank, where you have a well defined expectation of the
  data to be considered and reliability is arguably the most important feature. In
  this situation a document store would be a downgrade from SQL.  
  
- What is "NewSQL", and what is it trying to achieve?
NewSQL is a relational DBMS that is attempting to combine the scalability of NoSQL systems
and still maintain the reliability, the ACID guarantees, of traditional DBMSs.
